...
Sub Кнопка1_Щелчок()
  ...
  dataSheetName = "info"
  delimeter = Workbooks(ITK).Worksheets(dataSheetName).Range("J25").Value
  lastColumnNumSource = Workbooks(ITK).Worksheets(dataSheetName).Range("J26").Value
  ...
  For i = 0 To deptsNumberDetected - 1
    ...
    If CheckFileExists(currentFileName) Then
        ...
        resultStr = SendData(currentFileName, _
